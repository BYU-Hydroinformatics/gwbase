...
OUT_DIR = RESULTS / "deseason_qtr_delta"
...
clim = (qtr.groupby(["well_id","gage_id","quarter"])
        ...
        .reset_index())
qtr = qtr.merge(clim, on=["well_id","gage_id","quarter"])
...
qtr["delta_wte"] = qtr.groupby(["well_id","gage_id"])["wte_anom"].diff()
qtr["delta_q"]   = qtr.groupby(["well_id","gage_id"])["q_anom"].diff()
qtr["idx_diff"]  = qtr.groupby(["well_id","gage_id"])["qtr_idx"].diff()
...
before = len(qtr)
qtr = qtr[qtr.groupby("gage_id")["delta_wte"].transform(iqr_mask)].copy()
print(f"  Outlier removal (IQR×3 per gage on ΔWTE): {before - len(qtr)} rows removed")
...
reg_df.to_csv(FEAT_DIR / "regression_deseason_qtr_by_well.csv", index=False)
print(f"  Saved regression  ({len(reg_df)} rows)")

print("\n=== Deseasonalized quarterly delta — regression summary ===")
...
# ── Figures (same structure as quarterly_consec) ──────────────────────────────
...
for gname, grp in reg_df.groupby("gage_name"):
    ...
    sub = qtr[(qtr["gage_name"]==gname) & (qtr["well_id"].isin(top_ids))
...
pd.concat(tables).reset_index(drop=True).to_csv(
    TAB_DIR / "top10_deseason_qtr_by_r2.csv", index=False)
print(f"\nAll outputs → {OUT_DIR}")
